 ...
 * to fit the file system, and some Windows hosts fail earlier. Pure PHP;
 * the SQL that creates and drops the tables lives in the repository.
 */
final class TempTables {
	 ...
	 * @return string At most MAX_NAME bytes; over-long names are truncated and given a short hash.
	...
	public static function name( string $token, int $job_id, string $random, string $table ): string {
		...
		return $prefix . substr( $table, 0, $room ) . '_' . $hash;
	}
